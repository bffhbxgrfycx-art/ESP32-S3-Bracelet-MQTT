...
void lvgl_port_lock_init(void) {
    if(lvgl_mutex == NULL) {
        ...
        lvgl_mutex = xSemaphoreCreateRecursiveMutexStatic(&lvgl_mutex_buf);        // 修复：传递静态缓冲区给函数
        if(lvgl_mutex == NULL) {
            ESP_LOGE(TAG_LVGL, "Failed to create LVGL recursive mutex!");
        }
    }
}
bool lvgl_port_lock(uint32_t timeout_ms) {
    ...
    if(xSemaphoreTakeRecursive(lvgl_mutex, timeout_ticks) == pdTRUE) {
        ...
        ESP_LOGE(TAG_LVGL, "LVGL lock timeout!");
        return false;
    }
}
void lvgl_port_unlock(void) {
    if(lvgl_mutex != NULL) {
        xSemaphoreGiveRecursive(lvgl_mutex);        // 递归解锁
    }
}
...
/*Read the touchpad (no touch in this demo, always released)*/
